...
static __declspec(thread) UINT LastError;
static __declspec(thread) BOOL NonInteractive;
...
PUBLIC void
VDIW_SetNonInteractive(BOOL enabled)
{
...
VDIW_Create(CPFN fn, UINT BlockSize, UINT nBlocks, UINT nBlocksUsed)
{
   if (CheckDiskSpace(fn,BlockSize,nBlocksUsed)) {
      FILE f = File_Create(fn,DJFILE_FLAG_SEQUENTIAL);
      ...
      if (f == NULLFILE) {
         ...
         if (IOR == DJFILE_ERROR_ACCESS_DENIED) LastError = VDIW_ERR_ACCDENY;
            ...
            LastError = VDIW_ERR_EXISTS;
             if (!NonInteractive && Env_AskYN(RSTR(VWEXISTSQ), RSTR(VWEXISTSC))) {
               f = File_Create(fn,DJFILE_FLAG_SEQUENTIAL|DJFILE_FLAG_OVERWRITE);
...
PUBLIC BOOL
VDIW_Commit(HVDIW hVDI)
{
   LastError = 0;
   if (!hVDI) { LastError=VDIW_ERR_HANDLE; return FALSE; }
   {
      PVDI pVDI = (PVDI)hVDI;
      if (pVDI->committed) return TRUE;
      if (!File_Flush(pVDI->f)) { LastError=VDIW_ERR_WRITE; return FALSE; }
      if (!pVDI->blockmap) WriteHeader(pVDI);
      else {
         File_Seek(pVDI->f, sizeof(VDI_PREHEADER));
         if (File_WrBin(pVDI->f, &pVDI->hdr, sizeof(VDI_HEADER))!=sizeof(VDI_HEADER)) { LastError=VDIW_ERR_WRITE; return FALSE; }
         File_Seek(pVDI->f, pVDI->hdr.offset_Blocks);
         if (File_WrBin(pVDI->f, pVDI->blockmap, pVDI->hdr.nBlocks*sizeof(UINT))!=pVDI->hdr.nBlocks*sizeof(UINT)) { LastError=VDIW_ERR_WRITE; return FALSE; }
      }
      if (!File_Flush(pVDI->f)) { LastError=VDIW_ERR_WRITE; return FALSE; }
      pVDI->committed=TRUE;
   }
...
VDIW_Close(HVDIW hVDI)
{
   ...
   if (hVDI) {
      PVDI pVDI = (PVDI)hVDI;
      if (!pVDI->committed && !VDIW_Commit(hVDI)) return hVDI;
      File_Close(pVDI->f);
      if (File_IOresult()) LastError=VDIW_ERR_WRITE;
