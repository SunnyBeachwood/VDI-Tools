...
static INIT_ONCE stringCacheInit=INIT_ONCE_STATIC_INIT;
static CRITICAL_SECTION stringCacheLock;

static BOOL CALLBACK InitStringCacheLock(PINIT_ONCE once, PVOID parameter, PVOID *context)
{
   (void)once; (void)parameter; (void)context;
   InitializeCriticalSection(&stringCacheLock);
   return TRUE;
}
...
PUBLIC UI32
Env_AskRiskYN(PSTR szMsg, PSTR szCaption)
{
   UI32 rslt = MessageBox(GetFocus(),szMsg,szCaption,MB_ICONWARNING|MB_YESNO|MB_DEFBUTTON2);
   return rslt==IDYES;
}
...
Env_SetLanguage(HINSTANCE hInstLangDLL, UINT idLanguage)
{
   ...
   if (idLanguage==1)      idDefaultLanguage = MAKELANGID(LANG_CHINESE,SUBLANG_CHINESE_SIMPLIFIED);
   else if (idLanguage==2) idDefaultLanguage = MAKELANGID(LANG_GERMAN,SUBLANG_GERMAN);
   else if (idLanguage==3) idDefaultLanguage = MAKELANGID(LANG_JAPANESE,SUBLANG_JAPANESE_JAPAN);
   else if (idLanguage==4) idDefaultLanguage = MAKELANGID(LANG_FRENCH,SUBLANG_FRENCH);
   else if (idLanguage==5) idDefaultLanguage = MAKELANGID(LANG_SPANISH,SUBLANG_SPANISH_MODERN);
   else if (idLanguage==6) idDefaultLanguage = MAKELANGID(LANG_CHINESE,SUBLANG_CHINESE_TRADITIONAL);
   else if (idLanguage==7) idDefaultLanguage = MAKELANGID(LANG_KOREAN,SUBLANG_KOREAN);
   else if (idLanguage==8) idDefaultLanguage = MAKELANGID(LANG_RUSSIAN,SUBLANG_RUSSIAN_RUSSIA);
   else if (idLanguage==9) idDefaultLanguage = MAKELANGID(LANG_PORTUGUESE,SUBLANG_PORTUGUESE_BRAZILIAN);
   else { idLanguage = 0; idDefaultLanguage = MAKELANGID(LANG_ENGLISH,SUBLANG_ENGLISH_UK); }
...
{
   InitOnceExecuteOnce(&stringCacheInit,InitStringCacheLock,NULL,NULL);
   EnterCriticalSection(&stringCacheLock);
   if (*psz == NULL) {
      LPCWSTR pwsz = (ids==0 ? NULL : FindStringResourceEx(hInstLang, ids, idDefaultLanguage));
      if (pwsz==NULL && idDefaultLanguage!=MAKELANGID(LANG_ENGLISH,SUBLANG_ENGLISH_UK)) {
         pwsz = FindStringResourceEx(hInstLang,ids,MAKELANGID(LANG_ENGLISH,SUBLANG_ENGLISH_UK));
      }
      if (pwsz==NULL || pwsz[0]==0) *psz = "";
      else {
         UINT len,bytes,alloc;
         len = pwsz[0];
         bytes = WideCharToMultiByte(CP_ACP,0,pwsz+1,len,NULL,0,NULL,NULL);
         alloc = (bytes+4) & ~3; // add space for NUL byte then round up to dword boundary.
          if (bytes==0 || alloc>STR_CACHE_INCR) {
             *psz = "";
             LeaveCriticalSection(&stringCacheLock);
             return *psz;
          }
         ...
         WideCharToMultiByte(CP_ACP,0,pwsz+1,len,pStrCache->buff+pStrCache->BytesUsed,bytes,NULL,NULL);
         pStrCache->buff[pStrCache->BytesUsed+bytes] = (CHAR)0; // NUL terminate.
