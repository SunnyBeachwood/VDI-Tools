...
FSys_OpenVolume(UINT PartCode, HVDDR hVDI, HUGE iLBA, HUGE cLBA, UINT cSectorSize)
{
   ...
   if (PartCode==7 || PartCode==0x42 || PartCode==0 || PartCode==0xEE) {
      if (NTFS_IsNTFSVolume(hVDI,iLBA)) return NTFS_OpenVolume(hVDI,iLBA,cLBA,cSectorSize);
   }
   if (PartCode==0x83 || PartCode==0 || PartCode==0xEE) {
      if (Extx_IsLinuxVolume(hVDI,iLBA)) return Extx_OpenVolume(hVDI,iLBA,cLBA,cSectorSize);
   }
   if (PartCode<0x100 || PartCode==0xEE) {
      if (FAT_IsFATVolume(hVDI,iLBA)) return FAT_OpenVolume(hVDI,iLBA,cLBA,cSectorSize);
   }
   return NULL;
}
