...
PUBLIC BOOL
MediaReg_HasVDIChild(S_UUID *pUUID)
{
   if (hmRegData && nRegEntries) {
      PMEDREG pmr=(PMEDREG)MemFile.GetPtr(hmRegData,0);
      UINT i;
      for (i=0;i<nRegEntries;i++,pmr++) {
         FILE f=File_OpenRead(pmr->filename);
         if (f!=NULLFILE) {
            VDI_PREHEADER ph;
            if (File_RdBin(f,&ph,sizeof(ph))==sizeof(ph) && ph.u32Signature==VDI_SIGNATURE && ph.u32Version==VDI_VERSION_1_1) {
               VDI_HEADER h;
               if (File_RdBin(f,&h,sizeof(h))==sizeof(h) && UUIDmatch(pUUID,&h.uuidLinkage)) {
                  File_Close(f); return TRUE;
               }
            }
            File_Close(f);
         }
