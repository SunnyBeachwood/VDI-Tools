...
static __declspec(thread) HWND hWndProgress;
...
Progress_Begin(HINSTANCE hInstRes, HWND hWndOwner, PPROGINF pProg)
{
   ...
   pProg->LastUpdateTick = 0;
   Task_ReportProgress(pProg->BytesDone,pProg->BytesTotal);

   if (!pProg->bNoUI && !hWndProgress) {
      while (YieldCPU());
...
Progress_UpdateStats(PPROGINF pProg)
{
   ...
   if (hWndProgress) {
      ...
      if (pProg->LastUpdateTick && (DWORD)(now-pProg->LastUpdateTick)<200 && pProg->BytesDone<pProg->BytesTotal) return;
      pProg->LastUpdateTick=now;
...
{
   Task_ReportProgress(pProg->BytesTotal,pProg->BytesTotal);
   if (hWndProgress) {
