...
{
   FILE f=File_Open(c->journal);
   if (f==NULLFILE) { SetError("Could not update the in-place recovery journal."); return FALSE; }
   c->j.headerCrc=HeaderCrc(&c->j);
   File_Seek(f,0);
   if (File_WrBin(f,&c->j,sizeof(c->j))!=sizeof(c->j) || !File_Flush(f)) {
      File_Close(f); SetError("Could not flush the in-place recovery journal."); return FALSE;
   }
   File_Close(f);
   return TRUE;
}
...
static BOOL CreateJournal(VDIIP_CONTEXT *c)
{
   ...
   c->j.headerCrc=HeaderCrc(&c->j);
   f=File_Create(c->journal,DJFILE_FLAG_WRITETHROUGH);
...
static BOOL Analyze(VDIIP_CONTEXT *c, s_CLONEPARMS *parm)
{
   ...
   if (MediaReg_HasVDIChild(&h.uuidCreate)) {
