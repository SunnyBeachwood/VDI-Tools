...
#define MAX_BURST_BLOCKS 32

#define BLOCK_SIZE             1048576 /* must be a power of 2, and at least 512 */
#define SECTORS_PER_BLOCK      2048
#define SECTORS_PER_BURST      (2048*MAX_BURST_BLOCKS)
#define SPB_SHIFT              11      /* sectors per block again, but expressed as a shift */
#define MAX_MAPPED_PARTITIONS  32

static __declspec(thread) FILE          stderr;

static __declspec(thread) char          szfnSrc[4096];
static __declspec(thread) char          szfnDest[4096];
static __declspec(thread) HVDDR         SourceDisk;
static __declspec(thread) HVDIW         hVDIdst;
static __declspec(thread) HFSYS         pFSys[MAX_MAPPED_PARTITIONS];
static __declspec(thread) ProgInfo      prog;
static __declspec(thread) BOOL           BatchMode;
static __declspec(thread) char           LastErrorText[1024];
...
Error(PSTR sz)
{
   lstrcpyn(LastErrorText,sz,sizeof(LastErrorText));
   if (stderr) {
...
static UINT
MapPartitions(HFSYS fsMap[MAX_MAPPED_PARTITIONS], s_CLONEPARMS *parm)
// This function checks each partition in turn, and if it uses a supported guest filesystem
// then I create a filesystem object which maps that partition into used/unused clusters.
// Supports standard MBR partitions and GPT partitions.
{
   int i,j=0;
   for (i=0; i<MAX_MAPPED_PARTITIONS; i++) fsMap[i] = NULL;
   if (parm->flags & PARM_FLAG_COMPACT) {
      if (parm->MBR[510]==0x55 && parm->MBR[511]==0xAA) {
         ...
         for (i=0; i<4; i++) {
            if (pPart[i].PartType == 0xEE) {
               BYTE gptHdr[512];
               if (SourceDisk->ReadSectors(SourceDisk,gptHdr,1,1)!=VDDR_RSLT_FAIL && Mem_Compare(gptHdr,"EFI PART",8)==0) {
                  isGPT = TRUE;
                  break;
               }
            }
         ...
         if (isGPT) {
            ...
            if (SourceDisk->ReadSectors(SourceDisk,gptHdr,1,1)!=VDDR_RSLT_FAIL && Mem_Compare(gptHdr,"EFI PART",8)==0) {
               ...
               if (entriesLBA>0 && nEntries>0 && cbEntry>=128 && cbEntry<=512) {
                  ...
                  for (i=0; i<(int)nEntries && j<(MAX_MAPPED_PARTITIONS-1); i++) {
                     BYTE *pEntry;
                     HUGE firstLBA, lastLBA;
                     if ((i % perSec) == 0) {
                        if (SourceDisk->ReadSectors(SourceDisk,secBuffer,entriesLBA+(i/perSec),1)==VDDR_RSLT_FAIL) break;
                     }
                     pEntry = secBuffer + (i % perSec) * cbEntry;
                     if (!IsNullGuid(pEntry)) {
                        firstLBA = ReadLe64(pEntry+32);
                        lastLBA  = ReadLe64(pEntry+40);
                        if (firstLBA>0 && lastLBA>=firstLBA) {
                           cLBA = lastLBA - firstLBA + 1;
                           h = FSys_OpenVolume(0,SourceDisk,firstLBA,cLBA,512);
                           if (h) fsMap[j++] = h;
                        }
                     }
                  }
            ...
            for (i=0; i<4; i++) {
               ...
               h = FSys_OpenVolume(pPart->PartType,SourceDisk,startLBA,cLBA,512);
               if (h) fsMap[j++] = h;
               if (j==MAX_MAPPED_PARTITIONS-1) break;
               pPart++;
            }
         }
...
// and writes them to the dest drive if so.
{
   ...
   UINT burstBlocks=Task_CurrentIOBurstBlocks();
   BYTE  *buffer,*block;
   int   BlockStatus[MAX_BURST_BLOCKS];
   HUGE  LBA = 0;

   // to reduce seek overheads I read a "burst" of blocks then write
   // them as a burst too.
   if (burstBlocks<4) burstBlocks=4;
   if (burstBlocks>MAX_BURST_BLOCKS) burstBlocks=MAX_BURST_BLOCKS;
   buffer = Mem_Alloc(0,BLOCK_SIZE*burstBlocks);
   if (!buffer) {
      Error(RSTR(LOMEM));
      return FALSE;
   }
   ...
   for (iPage=iBurst=0; iPage<nPages; iPage++) { // iPage a.k.a. block number.
      ...
      if (iBurst==burstBlocks || (iPage+1)==nPages) {
         ...
         for (i=0; i<iBurst; i++,DestPage++) {
            ...
            if (BlockStatus[i] != VDDR_RSLT_NOTALLOC) {
               ...
               if (BlockStatus[i]==VDDR_RSLT_NORMAL) {
                  ...
                  Progress.UpdateStats(&prog);
                   if (prog.bUserCancel || Task_CurrentCancelRequested()) {
...
// HVDDR cow;
   ...
   BatchMode = (ParmFlags & PARM_FLAG_BATCHMODE)!=0;
   LastErrorText[0]=0;
   ...
   if (!hVDIdst) {
      ...
      if (!bSuccess) VDIW_Discard(hVDIdst);
         ...
         if (!VDIW_Commit(hVDIdst)) {
            Error(VDIW_GetErrorString(0xFFFFFFFF));
            VDIW_Discard(hVDIdst);
            bSuccess=FALSE;
         } else {
            VDIW_Close(hVDIdst);
            if (VDIW_GetLastError()!=VDIW_ERR_NONE) {
               File_Erase(szfnDest);
               bSuccess=Error(VDIW_GetErrorString(0xFFFFFFFF));
            }
         }
         if (!bSuccess) {
            /* Commit or final close failed; keep no incomplete destination. */
         } else {
   ...
   if (bSuccess) {
       if (!(parm->flags & (PARM_FLAG_CLIMODE|PARM_FLAG_BATCHMODE))) PlaySound("notify.wav", NULL, SND_FILENAME);
   }
   Progress.End(&prog);
   return bSuccess;
}
