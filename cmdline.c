...
#define MAX_BATCH_SOURCES 128
static FNCHAR BatchSources[MAX_BATCH_SOURCES][1024];
static UINT BatchSourceCount;
static UINT BatchThreadLimit;
static FNCHAR BatchOutputDir[1024];
...
static PSTR pszINPLACECONFLICT;
static PSTR pszINPLACENEEDSOP;
static PSTR pszFORCECONFLICT;
static PSTR pszRESUMECONFLICT;
...
static PSTR pszVOPTINPLACE    = "inplace";
static PSTR pszVOPTFORCEENC   = "force-encrypted";
static PSTR pszVOPTRESUME     = "resume";
static PSTR pszVOPTTHREADS    = "threads";
static PSTR pszVOPTOUTPUTDIR  = "output-dir";
static PSTR pszVOPTHELP       = "help";
static PSTR pszCHAROPT        = "okechri";
...
{
   CHAR szErr[256];
   wsprintf(szErr,RSTR(ARGERR),iArg,pszErr);
   Error(szErr);
   return Usage(FALSE);
}
...
{
   CHAR szErr[256];
   wsprintf(szErr,RSTR(OPTTWICE),iArg,pszOptName);
   Error(szErr);
   return Usage(FALSE);
}
...
static UINT
GetThreadsOption(UINT iArg)
{
   PSTR value=Env_ParamStr(iArg);
   UINT n=0;
   if (!value) return 0;
   if (String_Compare(value,"auto")==0) { BatchThreadLimit=0; return iArg+1; }
   while (*value>='0' && *value<='9') { n=n*10+(*value-'0'); value++; }
   if (*value || n<1 || n>32) return 0;
   BatchThreadLimit=n;
   return iArg+1;
}

static UINT
GetOutputDirOption(UINT iArg)
{
   PSTR value=Env_ParamStr(iArg);
   if (!value) return 0;
   GetFullPathName(value,1024,BatchOutputDir,0);
   return iArg+1;
}
...
{
   BOOL gotSrcFn=FALSE,gotDstFn=FALSE,gotInplace=FALSE;
   PSTR pszArg;
   UINT argc = Env_ParamCount();
   UINT iArg = 1;

   ZeroMemory(parm,sizeof(s_CLONEPARMS));
   ZeroMemory(BatchSources,sizeof(BatchSources));
   ZeroMemory(BatchOutputDir,sizeof(BatchOutputDir));
   BatchSourceCount=0; BatchThreadLimit=0;
         ...
         if (pszArg[1]=='-') {
            ...
            for (;;) {
               ...
               if (String_Compare(szItem,pszVOPTHELP)==0) {
                  ...
                  if (iArg==0) return FALSE;
                } else if  (String_Compare(szItem,pszVOPTINPLACE)==0) {
                   if (gotInplace) return ErrOptionSetTwice(pszVOPTINPLACE,iArg-1);
                   parm->flags|=PARM_FLAG_INPLACE; gotInplace=TRUE;
               } else if  (String_Compare(szItem,pszVOPTFORCEENC)==0) {
                  if (!GetOption(parm,iArg,PARM_FLAG_FORCE_ENCRYPTED,pszVOPTFORCEENC)) return FALSE;
                } else if  (String_Compare(szItem,pszVOPTRESUME)==0) {
                   if (!GetOption(parm,iArg,PARM_FLAG_RESUME,pszVOPTRESUME)) return FALSE;
                   parm->flags |= PARM_FLAG_INPLACE;
                } else if  (String_Compare(szItem,pszVOPTTHREADS)==0) {
                   iArg=GetThreadsOption(iArg);
                   if (!iArg) return ArgError("threads must be auto or an integer from 1 to 32",iArg-1);
                } else if  (String_Compare(szItem,pszVOPTOUTPUTDIR)==0) {
                   iArg=GetOutputDirOption(iArg);
                   if (!iArg) return ArgError(RSTR(NOFILEN),iArg-1);
                } else {
          ...
          if (BatchSourceCount>=MAX_BATCH_SOURCES) return ArgError("too many source files (maximum 128)",iArg-1);
          GetFullPathName(pszArg,1024,BatchSources[BatchSourceCount],0);
          if (!gotSrcFn) {
             String_Copy(parm->srcfn,BatchSources[BatchSourceCount],1024);
             gotSrcFn = TRUE;
          }
          BatchSourceCount++;
      }
   ...
   if (parm->flags & PARM_FLAG_RESUME) {
      UINT invalid=parm->flags&(PARM_FLAG_KEEPUUID|PARM_FLAG_ENLARGE|PARM_FLAG_REPART|PARM_FLAG_COMPACT);
      if (gotDstFn || invalid) return ArgError(RSTR(RESUMECONFLICT),iArg-1);
      parm->flags |= PARM_FLAG_FORCE_ENCRYPTED; /* authorization was recorded when the journal was created */
   }
   if (parm->flags & PARM_FLAG_INPLACE) {
      if (gotDstFn || BatchOutputDir[0] || (parm->flags&(PARM_FLAG_KEEPUUID|PARM_FLAG_REPART))) return ArgError(RSTR(INPLACECONFLICT),iArg-1);
      if (!(parm->flags&(PARM_FLAG_COMPACT|PARM_FLAG_ENLARGE|PARM_FLAG_RESUME))) return ArgError(RSTR(INPLACENEEDSOP),iArg-1);
      String_Copy(parm->dstfn,parm->srcfn,1024);
   } else {
      if (parm->flags&PARM_FLAG_FORCE_ENCRYPTED) return ArgError(RSTR(FORCECONFLICT),iArg-1);
      if (BatchSourceCount>1 && gotDstFn) return ArgError("--output is only valid with one source file",iArg-1);
      if (!gotDstFn) Env_GenerateCloneName(parm->dstfn,parm->srcfn);
...
PUBLIC UINT CmdLine_BatchSourceCount(void) { return BatchSourceCount; }
PUBLIC CPFN CmdLine_BatchSource(UINT index) { return index<BatchSourceCount ? BatchSources[index] : NULL; }
PUBLIC UINT CmdLine_BatchThreadLimit(void) { return BatchThreadLimit; }
PUBLIC CPFN CmdLine_BatchOutputDir(void) { return BatchOutputDir[0] ? BatchOutputDir : NULL; }
