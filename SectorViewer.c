...
InitDialog(HWND hDlg)
{
   ...
   { SCROLLINFO si; ZeroMemory(&si,sizeof(si)); si.cbSize=sizeof(si); si.fMask=SIF_RANGE|SIF_PAGE|SIF_POS; si.nMin=0; si.nMax=100000; si.nPage=1000; si.nPos=0; SetScrollInfo(GetDlgItem(hDlg,IDD_VSCROLL),SB_CTL,&si,TRUE); }
...
GotoSector(HWND hDlg, HVDDR hVDI, UINT nNewSector)
{
   if (nNewSector!=nSector) {
      ...
      { SCROLLINFO si; ZeroMemory(&si,sizeof(si)); si.cbSize=sizeof(si); si.fMask=SIF_POS; si.nPos=(TotalSectors>1 ? (int)(((double)nSector*100000.0)/(double)(TotalSectors-1)) : 0); SetScrollInfo(GetDlgItem(hDlg,IDD_VSCROLL),SB_CTL,&si,TRUE); }
      ShowSector(hDlg);
   }
}
...
SectorViewerDlgProc(HWND hDlg, UINT iMsg, WPARAM wParam, LPARAM lParam)
{
   ...
   switch (iMsg) {
      ...
      case WM_VSCROLL: {
         ...
         } else if (wNotify==SB_THUMBTRACK || wNotify==SB_THUMBPOSITION) {
            SCROLLINFO si; ZeroMemory(&si,sizeof(si)); si.cbSize=sizeof(si); si.fMask=SIF_TRACKPOS; GetScrollInfo(GetDlgItem(hDlg,IDD_VSCROLL),SB_CTL,&si);
            nNextSector=(TotalSectors>1 ? (UINT)(((double)si.nTrackPos*(double)(TotalSectors-1))/100000.0) : 0);
         }
